...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  {
    key: 'conversation.input.model.tools',
    kind: 'list',
    scope: 'session',
    summary: 'Companion controls rendered immediately RIGHT of the model select, still before the send button — the additive way to put a small tool-row action beside the model affordance without touching the plan/model seats.',
    doc: 'Companion controls rendered immediately RIGHT of the model select, still\nbefore the send button — the additive way to put a small tool-row action\nbeside the model affordance without touching the plan/model seats. Same\n`locked`-only owner share as those seats; entries render by ascending\n`order`, and the seat renders nothing while empty.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    declaredBy: 'an entry in \'conversation.composer.bar\' (client-ui-conversation), so it exists while that entry is mounted',
    occupants: [
      'client-ui-prompt-optimizer PromptOptimizeButton id \'prompt-optimizer\'',
    ],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'conversation.input.model.tools\', () => ctx.slots.register(\n      { name: \'conversation.input.model.tools\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
